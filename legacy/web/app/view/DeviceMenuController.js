...
Ext.define('Traccar.view.DeviceMenuController', {
  ...
  init: function () {
    ...
    this.lookupReference('menuCommandsButton').setHidden(
    ...
    this.lookupReference('menuDeviceAccumulatorsButton').setHidden(
      (!Traccar.app.getUser().get('administrator') &&
        Traccar.app.getUser().get('userLimit') === 0) ||
        Traccar.app.getVehicleFeaturesDisabled()
    );
  ...
  onDeviceAccumulatorsClick: function () {
    var position,
      dialog = Ext.create('Traccar.view.dialog.DeviceAccumulators');
    ...
    if (position) {
      dialog
        .lookupReference('totalDistance')
        .setValue(position.get('attributes').totalDistance);
      dialog
        .lookupReference('hours')
        .setValue(position.get('attributes').hours);
    }
    dialog.show();
  },
});
