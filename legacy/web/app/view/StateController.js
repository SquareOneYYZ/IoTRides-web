...
Ext.define('Traccar.view.StateController', {
  ...
  init: function () {
    ...
    if (
      ...
      (!Traccar.app.getUser().get('deviceReadonly') &&
        !Traccar.app.getPreference('readonly', false))
    ) {
      this.lookupReference('computedAttributesButton').setDisabled(
        Traccar.app.getBooleanAttributePreference(
          'ui.disableComputedAttributes'
        )
      );
    }
    hideAttributesPreference = Traccar.app.getAttributePreference(
  ...
  findAttribute: function (record) {
    return (
      record.get('deviceId') === this.position.get('deviceId') &&
      record.get('attribute') === this.lookupAttribute
    );
  },
  ...
  updatePosition: function () {
    ...
    for (key in this.position.data) {
      if (
        this.position.data.hasOwnProperty(key) &&
        this.keys[key] !== undefined
      ) {
        store.add(
          Ext.create('Traccar.model.Attribute', {
            priority: this.keys[key].priority,
            name: this.keys[key].name,
            attribute: key,
            value: Traccar.AttributeFormatter.getFormatter(key)(
              this.position.get(key)
            ),
          })
        );
      }
      ...
      for (key in attributes) {
        if (attributes.hasOwnProperty(key) && !this.hideAttributes[key]) {
          ...
          if (this.position.get('attribute.' + key) !== undefined) {
            value = Traccar.AttributeFormatter.getAttributeFormatter(key)(
              this.position.get('attribute.' + key)
            );
          } else {
            value = Traccar.AttributeFormatter.defaultFormatter(
              attributes[key]
            );
          }
          store.add(
  ...
  selectDevice: function (device) {
    ...
    position = Ext.getStore('LatestPositions').findRecord(
      'deviceId',
      this.deviceId,
      0,
      false,
