...
Ext.define('Traccar.view.map.MapMarkerController', {
  ...
  init: function () {
    ...
    this.liveRoutes = {};
    this.liveRouteLength = Traccar.app.getAttributePreference(
      'web.liveRouteLength',
      10
    );
  ...
  updateDevice: function (store, data) {
    ...
    for (i = 0; i < data.length; i++) {
      ...
      if (deviceId in this.latestMarkers) {
        ...
        if (
          style.getImage().fill !== this.getDeviceColor(device) ||
          style.getImage().category !== device.get('category')
        ) {
          this.updateDeviceMarker(
            style,
            this.getDeviceColor(device),
            device.get('category')
          );
          marker.changed();
        }
  ...
  removeDevice: function (store, data) {
    ...
    for (i = 0; i < data.length; i++) {
      ...
      if (this.latestMarkers[deviceId]) {
        if (
          markersSource.getFeatureById(this.latestMarkers[deviceId].getId())
        ) {
      ...
      if (this.accuracyCircles[deviceId]) {
        if (
          markersSource.getFeatureById(this.accuracyCircles[deviceId].getId())
        ) {
  ...
  animateMarker: function (marker, geometry, course) {
    ...
    updatePosition = function (position, marker) {
      ...
      coordinate = marker
        .get('line')
        .getCoordinateAt(position / (duration / timeout));
      style = marker.getStyle();
      ...
      if (marker === self.selectedMarker && follow) {
        self
          .getView()
          .getMapView()
          .setCenter(marker.getGeometry().getCoordinates());
      }
  ...
  updateAccuracy: function (position, device) {
    ...
    if (position.get('accuracy')) {
      ...
      radius = Ext.getStore('DistanceUnits').convertValue(
        ...
        Traccar.app.getAttributePreference('distanceUnit'),
        true
      );
      ...
      if (feature) {
        ...
        feature.setStyle(
          this.getAreaStyle(null, Traccar.Style.mapAccuracyColor)
        );
      ...
      if (
        feature &&
        this.getView().getAccuracySource().getFeatureById(feature.getId())
      ) {
  ...
  updateLatestMarker: function (position, device) {
    ...
    geometry = new ol.geom.Point(
      ol.proj.fromLonLat([position.get('longitude'), position.get('latitude')])
    );
    ...
    if (deviceId in this.latestMarkers) {
      ...
      if (
        ...
        this.lookupReference('deviceFollowButton').pressed
      ) {
        this.getView()
          .getMapView()
          .setCenter(marker.getGeometry().getCoordinates());
  ...
  updateLiveRoute: function (position, device) {
    ...
    if (deviceId in this.liveRoutes) {
      liveCoordinates = this.liveRoutes[deviceId]
        .getGeometry()
        .getCoordinates();
      lastLiveCoordinates = liveCoordinates[liveCoordinates.length - 1];
      newCoordinates = ol.proj.fromLonLat([
        position.get('longitude'),
        position.get('latitude'),
      ]);
      if (
        lastLiveCoordinates[0] === newCoordinates[0] &&
  ...
  loadReport: function (store, data) {
    ...
    if (data) {
      ...
      for (i = 0; i < data.length; i++) {
        ...
        if (
          i === 0 ||
          data[i].get('deviceId') !== data[i - 1].get('deviceId')
        ) {
          this.reportRoute.push(
            new ol.Feature({
              geometry: new ol.geom.LineString([]),
            })
          );
          this.reportRoute[this.reportRoute.length - 1].setStyle(
            this.getRouteStyle(data[i].get('deviceId'))
          );
          routeSource.addFeature(this.reportRoute[this.reportRoute.length - 1]);
        }
        this.reportRoute[this.reportRoute.length - 1]
          .getGeometry()
          .appendCoordinate(point);
      }
    }
  },
  ...
  getReportMarker: function (deviceId, angle) {
    return this.getMarkerStyle(
      false,
      Traccar.app.getReportColor(deviceId),
      angle,
      'arrow'
    );
  },
  ...
  rotateMarker: function (style, angle) {
    style.setImage(
      Traccar.DeviceImages.getImageIcon(
        style.getImage().fill,
        style.getImage().zoom,
        angle,
        style.getImage().category
  ...
  selectMarker: function (marker, center) {
    if (this.selectedMarker) {
      if (this.selectedMarker.get('event')) {
        ...
        !Ext.getStore('ReportRoute').showMarkers &&
        this.selectedMarker.get('record') instanceof Traccar.model.Position
      ) {
        this.getView().getMarkersSource().removeFeature(this.selectedMarker);
    ...
    if (marker) {
      ...
      if (center) {
        ...
        if (
          this.selectZoom !== 0 &&
          this.selectZoom > this.getView().getMapView().getZoom()
        ) {
  ...
  updateDeviceVisibility: function (device) {
    ...
    if (this.isDeviceVisible(device)) {
      if (
        marker &&
        !this.getView().getMarkersSource().getFeatureById(marker.getId())
      ) {
        this.getView().getMarkersSource().addFeature(marker);
      }
      if (
        accuracy &&
        !this.getView().getAccuracySource().getFeatureById(accuracy.getId())
      ) {
        this.getView().getAccuracySource().addFeature(accuracy);
      }
      if (
        liveLine &&
        !this.getView().getLiveRouteSource().getFeatureById(liveLine.getId())
      ) {
