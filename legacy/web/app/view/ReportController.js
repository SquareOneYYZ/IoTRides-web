...
Ext.define('Traccar.view.ReportController', {
  ...
  init: function () {
    ...
    for (i = 0; i < data.length; i++) {
      ...
      this.routeColumns.push({
        ...
        renderer: Traccar.AttributeFormatter.getAttributeFormatter(
          attribute.get('key')
        ),
        hidden: true,
      });
  ...
  onConfigureClick: function () {
    ...
    dialog
      .lookupReference('eventTypeField')
      .setHidden(
        this.lookupReference('reportTypeField').getValue() !== 'events'
      );
    dialog
      .lookupReference('chartTypeField')
      .setHidden(
        this.lookupReference('reportTypeField').getValue() !== 'chart'
      );
    ...
    if (this.eventType !== undefined) {
      ...
      dialog
        .lookupReference('eventTypeField')
        .setValue([Traccar.store.ReportEventTypes.allEvents]);
    }
  ...
  updateButtons: function () {
    ...
    devices =
      (this.deviceId && this.deviceId.length !== 0) ||
      (this.groupId && this.groupId.length !== 0);
    ...
    this.lookupReference('exportButton').setDisabled(
      reportType === 'chart' || disabled
    );
    this.lookupReference('emailButton').setDisabled(
      reportType === 'chart' || disabled
    );
  },
  ...
  onSelectionChange: function (selection, selected) {
    ...
    if (selected.length > 0) {
      ...
      if (
        report instanceof Traccar.model.Event ||
        report instanceof Traccar.model.ReportStop
      ) {
  ...
  selectReport: function (object) {
    ...
    if (object instanceof Traccar.model.Position) {
      if (reportType === 'route') {
        ...
        positionRelated = this.getGrid()
          .getStore()
          .findRecord('positionId', object.get('id'), 0, false, true, true);
        this.getGrid()
          .getSelectionModel()
          .select([positionRelated], false, true);
  ...
  loadRoute: function (store) {
    ...
    if (this.lookupReference('reportTypeField').getValue() === 'chart') {
      this.getChart()
        .getAxes()[0]
        .setTitle(
          Ext.getStore('ReportChartTypes')
            .findRecord('key', this.chartType)
            .get('name')
        );
      chartSeries = [];
  ...
  routeColumns: [
    ...
    },
    {
      ...
      renderer: function (value, metaData, record) {
        if (!value) {
          return (
            '<a href="#" onclick="Ext.fireEvent(\'routegeocode\', ' +
            record.getId() +
            ')" >' +
