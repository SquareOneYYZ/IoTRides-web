...
const MapRoutePoints = ({ positions, onClick }) => {
  ...
  useEffect(() => {
    ...
    const minSpeed = Math.min(...positions.map((p) => p.speed));
    ...
    const setNearbyPoints = (clickedFeature) => {
      const clickedIndex = clickedFeature.properties.index;
      const nearby = positions.filter(
        (_, i) => i >= clickedIndex - 10 && i <= clickedIndex + 10,
      );

      map.getSource(id)?.setData({
        type: 'FeatureCollection',
        features: nearby.map((p, index) => ({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [p.longitude, p.latitude] },
          properties: {
            index,
  ...
  }, [positions, simplifiedPositions, speedUnit, t, onClick]);
